...
def do_python_eval(output_dir, set_type,use_07=True):
    ...
    print('VOC22 metric? ' + ('Yes' if use_07_metric else 'No'))
...
# first load gt
    ...
    # read list of images
    with open("C://Users/25283/Desktop/SSD_FPN_GIoU/data/VOCdevkit/VOC2022/ImageSets/Main/train.txt", 'r') as f:
        lines = f.readlines()
...
def test_net(save_folder, net, cuda, dataset, top_k,im_size=300, thresh=0.05):
    ...
    for i in tqdm(range(num_images)):
        with torch.no_grad():
            im, gt, h, w, name = dataset.pull_item(i)
            x = im.unsqueeze(0)
            if args.cuda:
                x = x.cuda()
            _t['im_detect'].tic()
            ...
            sum_time = sum_time + detect_time

            # detections = detections[0, :, :]
            # boxes = detections[:, 1:]
            # print(gt_l.shape, print(boxes.shape))
            # skip j = 0, because it's the background class
            gt_l = gt[:, -1].astype(np.uint8)
            gt_l = set(gt_l)
            ...
            # for j in set(gt_l):
                ...
                pred_b.append(boxes)
                pred_s.append(torch.tensor(scores))
                pred_l.append(torch.tensor([j-1]*boxes.shape[0]))
            if len(pred_b) == 0:
                continue
            pred_b = torch.cat(pred_b).cpu().numpy()
            pred_s = torch.cat(pred_s).cpu().numpy()
            pred_l = torch.cat(pred_l).cpu().numpy()
            ...
            labeler = LabelFile(filename, img_path, labels)
    ...
    print('FPS={:.1f}'.format(num_images / sum_time))
    return all_boxes

def evaluate_detections(data_dir,box_list,dataset,eval_type = 'train'):
...
if __name__ == '__main__':
    ...
    net = build_ssd('train', size = 300, cfg = voc)            # initialize SSD
    net.load_state_dict(torch.load(args.trained_model))
    
    print('Finished loading model!')
    # load data
    dataset = VOCDetection(args.voc_root, [('2024', 'train')],
    ...
    # evaluation
    devkit_path = VOC_ROOT +'VOC2022/'

    all_boxes = test_net(args.save_folder, net, args.cuda, dataset,args.top_k, 300,
             thresh=args.confidence_threshold)
    # print('Evaluating detections')
    # result = evaluate_detections(devkit_path,all_boxes, dataset,'train')
